...
export function generateMetadata({ params }: { params: { contractor: string } }): Metadata {
  ...
  const description = `בדק בית הנדסי לדירות בפרויקטים של ${contractor.name} - בדיקת מסירה, איתור ליקויי בנייה וחוות דעת מקצועית.${projectsHint} מומחה מוסמך באיגוד המהנדסים.`
...
export default function ContractorPage({ params }: { params: { contractor: string } }) {
    ...
    ? [
        {
          ...
          paragraphs: [
            `בין הפרויקטים הידועים של ${contractor.name} שבהם אנו מבצעים בדק בית לדירות חדשות במעמד המסירה: ${projects.join("; ")}. בכל פרויקט אנו בודקים את הדירה באופן שיטתי — התאמה למפרט, ריצוף, איטום, אינסטלציה, חשמל ואלומיניום — ומתעדים כל ליקוי בדוח מקצועי.`,
            `אם רכשתם דירה באחד הפרויקטים של ${contractor.name}, מומלץ לבצע בדק בית במעמד המסירה — כדי לאתר ליקויים בזמן, בעוד תקופת הבדק והאחריות בתוקף, ולדרוש את תיקונם מהקבלן.`,
          ],
        },
      ]
    : []
  ...
  return (
    <SeoLandingTemplate
      ...
      subtitle={`בדק בית הנדסי מקצועי לדירות חדשות בפרויקטים של ${contractor.name}. בדיקת מסירה, איתור ליקויי בנייה וחוות דעת מקצועית מטעם מומחה מוסמך.`}
      intro={`רכשתם דירה בפרויקט של ${contractor.name}? במעמד מסירת הדירה — או סמוך לו ככל הניתן — חשוב לבצע בדק בית הנדסי מקצועי. מחקר מקיף שנערך בטכניון מצא כי כמעט בכל דירה חדשה בישראל מתגלים ליקויי בנייה, ולכן בדיקה הנדסית במועד המסירה היא הדרך הבטוחה להבטיח שתקבלו דירה תקינה ובאיכות שהובטחה לכם בחוזה.`}
      features={[
        {
          title: "בדיקת מסירה",
          description: `בדיקה הנדסית מקיפה של הדירה בפרויקט של ${contractor.name} במעמד המסירה, לאיתור כל הליקויים והחריגות.`,
        },
        {
      ...
      contentSections={[
        {
          heading: `על ${contractor.name} ובדק בית בפרויקטים שלה`,
          paragraphs: [
            contractor.description,
            `כמו אצל כל חברה קבלנית, גם בפרויקטים של ${contractor.name} עלולים להופיע ליקויי בנייה ואי-התאמות למפרט. בדק בית הנדסי עצמאי ובלתי תלוי במעמד המסירה מאתר את הליקויים ומתעד אותם בדוח קביל — כדי שתקבלו דירה תקינה ושהקבלן יתקן את הליקויים על חשבונו.`,
          ],
        },
        ...projectsSection,
        {
          heading: `מתי לבצע בדק בית בפרויקט של ${contractor.name}?`,
          paragraphs: [
            `נקודה שרבים טועים בה: רוב החברות הקבלניות אינן מאפשרות לבצע בדק בית שבועות לפני המסירה. הבדיקה נעשית במעמד המסירה עצמו או בסמוך לאחריה, ולכן יש לתאם את המומחה למועד המסירה — או קרוב אליו ככל האפשר.`,
            `העיתוי הזה אינו טכני בלבד אלא מהותי: כאשר הבדיקה סמוכה למסירה, הצוותים והחומרים של הקבלן עדיין זמינים והתיקון פשוט וזול עבורו, ולא נותר פתח לטעון שהליקוי נגרם משימוש של הדייר. כל ליקוי שמתועד נכנס לפרוטוקול המסירה ומחייב תיקון.`,
          ],
        },
        {
        ...
        },
        {
          ...
          paragraphs: [
            `ממצאי המחקר מהטכניון ממחישים עד כמה נפוצה תופעת ליקויי הבנייה בדירות חדשות בישראל. הנתונים מצביעים על כך שאיתור הליקויים מבעוד מועד, באמצעות בדק בית מקצועי במועד מסירת הדירה, חוסך לרוכשים עשרות אלפי שקלים ומונע התדיינויות משפטיות ממושכות מול הקבלן.`,
      ...
      faq={[
        {
          ...
          answer:
            "מומלץ לבצע את הבדיקה במועד מסירת הדירה הרשמית, כדי שתוכלו לדרוש תיקונים מהקבלן במעמד המסירה וחתימה על פרוטוקול המסירה.",
        },
        {
      ...
      ctaText={`צרו קשר עוד היום לתיאום בדק בית הנדסי מקצועי במועד מסירת הדירה. מומחה מוסמך, שירות מהיר וחוות דעת מקצועית.`}
